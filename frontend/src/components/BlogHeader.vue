            ...
            <div v-if="hasLogin">
                <div class="dropdown">
                    ...
                    <div class="dropdown-content">
                        ...
                        <router-link :to="{ name: 'HomeView' }" v-on:click.prevent="logout()">注销登录</router-link>
                    </div>
                </div>
            </div>
            <div v-else>
    ...
    export default {
        ...
        methods: {
          logout: function () {
            this.hasLogin = false;
            localStorage.clear();
            if (this.$route.name == "HomeView") { // 如果当前页面是首页，才要刷新，否则不用刷新，因为router-link已经跳转到首页去了，如果再刷新则又回到了前一级页面
